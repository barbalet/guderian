...
- **Cycles 61-80 complete:** Added shooting, vehicle, and close-quarters order-dice presenters to the visible battle command panel, exposed DZW vehicle state through snapshots, and routed Guderian-command automated steps through an activation planner that chooses unit, order, objective/target, path, and failed-order-test handling. Documentation lives in `docs/guderian_order_dice_cycle_061_080.md`; opposing-army AI, Ambush/Down/Rally choices, pins/morale AI, and target reactions remain next in cycles 81-100.
- **Cycles 81-100 complete:** Added opposing-force order-dice activation decisions across all 35 unified battles, deterministic Ambush/Down/Rally standing-order choices, pins/morale/FUBAR risk scoring, target-reaction decisions for both selected-side directions, and visible opposing-AI summaries in the battle command panel. Documentation lives in `docs/guderian_order_dice_cycle_081_100.md`; autoplay activation stepping and scenario tuning begin next in cycles 101-120.
- **Cycles 101-120 complete:** Converted GuderianTest and playable test automation to count order-dice activation steps while preserving pause/resume/speed/safety controls; added field-command scenario tuning rows for Poland, France, and the Eastern Front with pins, Rally/Down, road/rough movement, river/port/evacuation pressure, T-34/KV armour, logistics, winter, and morale emphasis; and surfaced tuning summaries in the battle command panel. Documentation lives in `docs/guderian_order_dice_cycle_101_120.md`; late-career tuning and activation-aware scoring continue in cycles 121-140.
- **Cycles 121-140 complete:** Added late-career order-dice tuning rows for all 16 staff/withdrawal/collapse/final-war records, added activation-aware scoring rows for all 35 unified battle entries, and surfaced late-career tuning plus score pacing in the battle command panel. Documentation lives in `docs/guderian_order_dice_cycle_121_140.md`; order-dice save-state migration, debrief copy updates, and activation-first tutorial language continue in cycles 141-160.
- **Cycles 141-160 complete:** Added order-dice save-state migration rows for all 35 unified battles, activation-first debrief copy that reports score plus activation pacing, tutorial language that teaches orders and activations before compatibility windows, and visible battle-panel identifiers for save/debrief migration state. Documentation lives in `docs/guderian_order_dice_cycle_141_160.md`; turn-end persistence hardening, replay signatures, full 35-battle harness reporting, and stale phase-surface retirement continue in cycles 161-180.
- **Cycles 161-180 complete:** Added turn-end order-dice persistence write sets for all 35 unified battles, deterministic replay/harness rows, activation balance audit rows, and a stale phase-surface retirement report that verifies `Next Window` player copy while retaining explicit compatibility identifiers. Documentation lives in `docs/guderian_order_dice_cycle_161_180.md`; Monty API handoff, build matrix verification, migration cleanup, and final acceptance continue in cycles 181-200.
- **Cycles 181-200 complete:** Published the Monty-facing order-dice API handoff, build-matrix ledger, migration-cleanup quarantine rows, and final acceptance closeout with known limitations and downstream requirements. Documentation lives in `docs/guderian_order_dice_cycle_181_200.md`; there are zero remaining Guderian order-dice migration cycles.
